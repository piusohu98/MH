...
阶段 3.0 的 Collector 只读取用户选择目录中的 `manifest.json` 和本地图片。它不上传截图、不调用市场服务，也不执行游戏操作。
...
`rawText` 和 `candidates` 目前用于离线调试与契约验证。真实 OCR 模型尚未接入，因此当前节点不代表已经完成真实图片识别，也没有真实识别率或 P95 性能结论。
