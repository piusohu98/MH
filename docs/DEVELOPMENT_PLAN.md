...
- OCR：先通过可注入的本地 recognizer 契约和确定性 Fake 完成离线回放验证；RapidOcrNet 中文模型只有在模型、字典和标注夹具核验后才能接入，全程本地处理。
...
| `MH.Client` | 只读 API 客户端、WPF 单商品走势/建议、活动观察、跨事件/跨区比较、区服代理指标和按鼠标所在显示器/DPI 校准的当前快照悬浮窗 | 屏幕 OCR 查价、多显示器/DPI 人工目视验收 |
| `MH.Collector` | 本地 manifest/图片目录回放、OCR 契约、确定性 Fake、可选商品目录匹配、checkpoint、逐帧进度、取消、复核提醒、人工复核决定 sidecar、证据导出和停止状态转移契约 | 中文 OCR、真实页面检测和端到端写入 |
...
后续 OCR 扩展需要增加原图本地路径或哈希、识别区域、文本、置信度、候选商品和人工确认状态。当前 `IOcrRecognizer` 只允许本地实现，Fake recognizer 只复用 manifest 中已有的文本/候选，不打开图片、不联网、不产生 UI 输入；可选 `catalog` 字段按目录名称匹配后再进入低置信度和人工确认分类。低置信度数据默认不进入建议计算；原始截图默认不离开本机，并支持按保留期清理。
...
阶段 2 额外验证确定性回测、时间边界和离线模式；WPF 第一屏和活动观察卡已完成人工验收。阶段 3 验证低置信度行为、截图不上传、悬浮窗原生工作区/DPI 定位和 P95；阶段 4 验证所有停止状态与端到端回放；阶段 5 验证自包含 Windows 发布。悬浮窗的多显示器/DPI 自动定位已有契约和构建验证，实际混合 DPI 桌面目视仍需人工运行证据。
