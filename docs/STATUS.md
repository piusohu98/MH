...
- 跨区事件标准化、WPF 活动的长期统计仍未实现；当前已完成单区单商品的跨事件事实归纳，但尚未把不同区服的绝对价格差异转换为可比较分位数。
...
推荐从阶段 2 的下一个最小闭环继续（人工界面验收已通过）：

1. 补充跨区事件标准化：同商品/类别按区服内基线转换为分位数，保留训练窗口、样本数和不可比原因。
2. 在跨区标准化通过确定性测试后，再做区服活跃/高价值需求代理指数和个人仓位。
...
```text
继续 MH 项目阶段 2 的下一小节：在已完成人工视觉验收和跨事件归纳的基础上，实现跨区事件标准化。
先阅读 docs/PROJECT_PLAN.md、docs/DEVELOPMENT_PLAN.md、docs/STATUS.md。
先阅读当前跨事件归纳实现和测试，再实现跨区标准化。后续实现必须保持单次活动事实、跨事件统计和跨区比较分离，不把历史样本归纳成必涨必跌或买卖结论，并继续保持 Release 构建与全量测试通过。
```
