...
结果：122 passed，0 failed，0 skipped
```

122 项测试覆盖：数据闭环、稳健指标、建议规则、5 类确定性虚拟行情、滚动回测、质量门禁、建议预览 API，以及客户端四端点、目录初始化/重试、空目录、无日线、中文/空格 URI、UTC 归一化、非法输入、首次失败、离线保留、主动取消、请求竞态、48 小时陈旧边界、地址覆盖、乱序最新价格和玩家展示全部分档边界。
