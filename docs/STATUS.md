...
结果：153 passed，0 failed，0 skipped

dotnet build MH.slnx -c Release --no-restore
结果：5/5 项目成功，0 warning，0 error
```

153 项测试覆盖：数据闭环、稳健指标、建议规则、5 类确定性虚拟行情、滚动回测、质量门禁、建议预览 API、活动事件前中后纯 Core 分析和事件 API，以及客户端四端点、目录初始化/重试、空目录、无日线、中文/空格 URI、UTC 归一化、非法输入、首次失败、离线保留、主动取消、请求竞态、48 小时陈旧边界、地址覆盖、乱序最新价格和玩家展示全部分档边界；本次补充了价格/在售数量独立比较、缺省窗口和 cutoff 隔离回归。
...
- WPF 活动日历、事件标记与跨区事件标准化尚未实现；本节点仅完成只读研究后端。
...
2. 后端合同和确定性测试稳定后，再把活动日历与事件标记接入现有走势图。
3. 活动节点验收后再做跨区标准化、区服活跃/高价值需求代理指数和个人仓位。
...
```text
继续 MH 项目阶段 2 的下一小节：把已完成的活动事件研究后端接入 WPF 活动日历和商品走势图。
先阅读 docs/PROJECT_PLAN.md、docs/DEVELOPMENT_PLAN.md、docs/STATUS.md。
先确认 WPF 第一屏人工验收已通过；若尚未通过，只修第一屏并停止。
验收通过后只改 MH.Client 和 MH.Tests，不改 Collector/OCR、事件分析口径、建议规则或数据库。客户端新增事件列表/影响只读 API 合同；在现有走势图标出活动区间，并增加玩家语言的活动卡，展示活动前、中、后的常见价格变化、在售数量变化、阶段是否完整、样本是否足够和不可用原因。默认只展示 Holiday/SupplyChange，DayNight 不应淹没日历；不得把单次活动结果直接描述成必涨、必跌或买卖结论。
成功标准：客户端 URI 编码/UTC、取消与竞态、离线保留、未来/进行中/样本不足文案测试通过；活动标记不破坏现有走势图和玩家首屏；Release 0 warning/0 error，全量测试通过。完成后停止，下一节点再做跨事件/跨区标准化。
```
