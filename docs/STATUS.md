...
1. 设计区服活跃度代理指数：先确定挂单新增、可见深度、价格修改和挂单消失等可采集字段，输出无量纲指数，不映射具体人数。
2. 再做高价值需求代理指数和个人仓位；继续保留数据新鲜度、样本数和不可用原因。
...
```text
继续 MH 项目阶段 2 的下一小节：在已完成人工视觉验收、跨事件归纳和跨区标准化的基础上，实现区服活跃度与高价值需求代理指标。
先阅读 docs/PROJECT_PLAN.md、docs/DEVELOPMENT_PLAN.md、docs/STATUS.md。
先阅读当前跨区标准化实现和测试，再实现区服代理指标。后续实现必须保持单次活动事实、跨事件统计、跨区比较和人数代理分离，不把无量纲指数宣称为具体人数或买卖结论，并继续保持 Release 构建与全量测试通过。
```
