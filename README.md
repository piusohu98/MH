...
- 209 项自动化测试覆盖数据闭环、稳健指标、定向虚拟场景、滚动回测、质量门禁、建议预览 API、活动影响研究、跨事件归纳、跨区标准化，以及客户端初始化、安全降级、事件 API、活动文案、图表事件区间、快照时点隔离和请求竞态。
...
2. WPF 行情面板：人工视觉验收、跨事件归纳和跨区事件标准化已完成；下一步做区服活跃/高价值需求代理指数与个人仓位。
