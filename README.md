...
- WPF 客户端第一屏已接通：自动加载目录和最新历史时点，提供区服/商品选择、价格折线、指标卡、建议门禁、离线/陈旧提示和重新初始化。采集器仍是可编译空壳。
- 91 项自动化测试，覆盖数据闭环、稳健指标、定向虚拟场景、滚动回测、质量门禁、建议预览 API，以及客户端初始化、URI、竞态、取消、离线和输入校验。
