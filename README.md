...
- 跨事件历史归纳已接入：按同一区服/商品/活动类型统计活动中与活动后的价格、在售数量中位变化、方向计数和一致度；少于 3 个可比较活动时只显示样本不足，不输出买卖结论。跨区标准化尚未实现。
- 188 项自动化测试，覆盖数据闭环、稳健指标、定向虚拟场景、滚动回测、质量门禁、建议预览 API、活动影响研究、跨事件归纳，以及客户端初始化、安全降级、事件 API、活动文案、图表事件区间、快照时点隔离和请求竞态。
...
2. WPF 行情面板：人工视觉验收已完成；下一步做跨区事件标准化，再做区服指标与个人仓位。
